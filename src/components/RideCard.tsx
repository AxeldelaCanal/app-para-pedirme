              ...
              {ride.pending_changes.destinations ? (
                <>
                  <div className="flex justify-between gap-2">
                    <span className="text-amber-700 dark:text-amber-400 font-medium">Paradas</span>
                    <span className="text-amber-900 dark:text-amber-200 font-semibold text-right">
                      {ride.pending_changes.destinations.map(d => d.address.split(',')[0]).join(' → ')}
                    </span>
                  </div>
                  {ride.pending_changes.price_ars && (
                    <div className="flex justify-between gap-2">
                      <span className="text-amber-700 dark:text-amber-400 font-medium">Nuevo precio</span>
                      <span className="text-amber-900 dark:text-amber-200 font-semibold">${ride.pending_changes.price_ars.toLocaleString('es-AR')}</span>
                    </div>
                  )}
                </>
              ) : (
                (Object.entries(ride.pending_changes) as [keyof PendingChanges, unknown][])
                  ...
                  .map(([key, val]) => {
                    const labels: Partial<Record<keyof PendingChanges, string>> = {
                      scheduled_at: 'Horario', origin: 'Origen', destination: 'Destino', price_ars: 'Precio',
                    }
                    return (
                      <div key={key} className="flex justify-between gap-2">
                        <span className="text-amber-700 dark:text-amber-400 font-medium">{labels[key]}</span>
                        <span className="text-amber-900 dark:text-amber-200 font-semibold text-right">{diffLabel(key, val)}</span>
                      </div>
                    )
                  })
              )}
